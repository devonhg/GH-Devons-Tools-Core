...
function MYPLUGIN_get_id_by_slug($page_slug, $pt) {
...
function MYPLUGIN_single( $archive = false , $display = null , $pt = null, $post_slug = null ){
    ...
    if ( is_string($post_slug) ){
    	$pID = MYPLUGIN_get_id_by_slug( $post_slug, $pt );
	}else{
		$pID = $post_slug;
	}
		    ...
		    if ($isCats){
		    	$out .= "<div class='" . "MYPLUGIN-categories" . "'>";
			    	foreach($pt_values as $tax){
			    		$out .= MYPLUGIN_get_cats( $tax , $post_slug, $pt);
			    	}
		    	$out .= "</div>";
	    	}
...
function MYPLUGIN_archive( $args , $wpargs = "" , $pt = null ){
    ...
    if ( basename($_SERVER['PHP_SELF']) != "post.php" ){
        ...
        while($the_query->have_posts()) : $the_query->the_post();
			...
			$out .= MYPLUGIN_single( true , $args , $pt , get_the_ID()  );
